...
> **Note:** `index_all.py` has the embed step commented out by default for local chunk-only usage.
> On RunPod, uncomment the two `embed_and_upsert` imports/calls in `index_all.py`
> before running, or run the full pipeline via `scripts/run_pipeline.sh`.
